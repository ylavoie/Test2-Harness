...
sub run_job {
    ...
    if ($via) {
        ...
        my $params = $job->run_params();
        $pid = $self->spawn($job, %$params);
    }
...
sub wait {
    ...
    my @jobsets = $self->SUPER::wait(%params);

    for my $set (@jobsets) {
        my ($job, $exit) = @$set;
        $self->write_exit($job, exit => $exit, stamp => time);
    }

    $self->unlock unless keys %{$self->{+PROCS}};
    delete $self->{+STATE_CACHE} if @jobsets;

    return scalar @jobsets;
}
...
sub write_exit {
    ...
    my $file = File::Spec->catfile($job->job_dir, 'exit');
