...
use Test2::Bundle::Extended -target => 'Test2::Harness::Worker';
...
subtest construction => sub {
    ...
    my $dir = tempdir("T2-Harness-XXXXXXXX", CLEANUP => 1);
...
subtest find_worker_inc => sub {
    ...
    ok(-e "$path/Test2/Harness/Worker.pm", "Found worker.pm inside it");
};
...
subtest spawn => sub {
    ...
    my $args;
    my $mock = mock 'Test2::Harness::Worker' => (
        override => [
...
subtest run_open3 => sub {
    ...
    is(
        ...
        array {
            ...
            item $^X;
            item '-w';
            item match qr/^-I/;    # Include dir
            item "-MTest2::Plugin::IOSync=" . $job->muxed_file->name;
            item __FILE__;
            end;
        },
        "Ran expected open3 command"
    ...
    # Test mux only, no switches
    $job->{test}           = mock {} => (add => [switches => sub { }]);
    $run->{output_merging} = 0;
    $run->{output_events}  = 0;
    $run->{output_muxing}  = 1;
    $run->{event_stream}   = 0;
    is($one->run_open3($job), 999_999_999, "Got pid");
    is(
        ...
        array {
            ...
            item $^X;
            item match qr/^-I/;    # Include dir
            item "-MTest2::Plugin::IOMuxer=" . $job->muxed_file->name;
            item __FILE__;
            end;
        },
        "Ran expected open3 command"
    ...
    # %ENV must always get restored!
    $control->override(open3 => sub { $open3_env = {%ENV}; die "foo" });
    like(dies { $one->run_open3($job) }, qr/foo/, "open3 died, excellent");
    like($open3_env, hash { field this_is_only_a_test => 'maybe?'; etc }, "Set the env for open3 before it died");
    like(\%ENV,      hash { field this_is_only_a_test => DNE();    etc }, "restored env hash despite exception");
};
...
subtest preload => sub {
    ...
    local @INC = ('t/lib', @INC);
    local %INC = %INC;
    delete $INC{'Test2/Plugin/IOSync.pm'};
    delete $INC{'Test2/Plugin/IOEvents.pm'};
    delete $INC{'Test2/Plugin/IOMuxer.pm'};
...

    {
        ...
        like(
            \%INC,
            {
                'Test2/Plugin/IOSync.pm'    => DNE(),
                'Test2/Plugin/IOEvents.pm'  => DNE(),
                'Test2/Plugin/IOMuxer.pm'   => DNE(),
...

    {
        ...
        like(
            ...
            {
                'Test2/Plugin/IOSync.pm'    => DNE(),
                'Test2/Plugin/IOEvents.pm'  => T(),
                'Test2/Plugin/IOMuxer.pm'   => DNE(),
                'Test2/Formatter/Stream.pm' => T(),
                'FakeModule.pm'             => T(),
            },
            "Loaded stream formatter and events"
        );
...

    {
        ...
        like(
            \%INC,
            {
                'Test2/Plugin/IOSync.pm'    => DNE(),
                'Test2/Plugin/IOEvents.pm'  => DNE(),
                'Test2/Plugin/IOMuxer.pm'   => T(),
                'Test2/Formatter/Stream.pm' => T(),
                'FakeModule.pm'             => T(),
            },
            "Loaded stream formatter and muxer"
        );
...

    {
        ...
        like(
            \%INC,
            {
                'Test2/Plugin/IOEvents.pm'  => DNE(),
                'Test2/Plugin/IOMuxer.pm'   => DNE(),
                'Test2/Plugin/IOSync.pm'    => T(),
                'Test2/Formatter/Stream.pm' => T(),
                'FakeModule.pm'             => T(),
            },
            "Loaded stream formatter and sync"
        );
...
subtest run_preloaded => sub {
    ...
    delete $INC{'Test2/Plugin/IOMuxer.pm'};
    delete $INC{'Test2/Formatter/Stream.pm'};
    local *Test2::Plugin::IOSync::import    = sub { $imports{iosync}   = [@_] };
    local *Test2::Plugin::IOEvents::import  = sub { $imports{ioevents} = [@_] };
    local *Test2::Plugin::IOMuxer::import   = sub { $imports{iomuxer}  = [@_] };
    ...
    my $ok     = eval {
        ...
        is(
            $imports{iomuxer},
            ['Test2::Plugin::IOMuxer', $job->muxed_file->name],
            "Imported Test2::Plugin::IOMuxer"
        );
        ...
        is(
            ...
            "Imported Test2::Plugin::IOEvents"
        );
        ...
        is(
            $imports{iosync},
            ['Test2::Plugin::IOSync', $job->muxed_file->name],
            "Imported Test2::Plugin::IOSync"
        );
...
subtest start => sub {
    ...
    my $c1 = mock 'Test2::Harness::Run' => (
        override => [
            find_tests => sub {
                Test2::Harness::TestFile->new(filename => 't/simple.t'),
                Test2::Harness::TestFile->new(filename => 't/simple.t'),
                Test2::Harness::TestFile->new(filename => 't/simple.t'),
                Test2::Harness::TestFile->new(filename => 't/simple.t'),
            },
