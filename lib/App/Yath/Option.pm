...
sub option_slot {
    ...
    confess "A settings hash is required" unless $settings;

    if (my $prefix = $self->{+PREFIX}) {
        return \($settings->{$prefix}->{$self->{+FIELD}});
    }

    return \($settings->{$self->{+FIELD}});
}
...
sub handle {
    ...
    return $self->{+ACTION}->($self->{+PREFIX}, $self->{+FIELD}, $raw, $norm, $slot, $settings)
        if $self->{+ACTION};

    return $$slot++
        if $self->{+TYPE} eq 'c';

    return push @{$$slot //= []} => $norm
        if $self->{+TYPE} eq 'm'
        || $self->{+TYPE} eq 'D';
...
stored in the settings hash. B<NOTE:> if the C<prefix> attribute is also set
...
The settings hash into which options are stored is a 2-level structure. There
are top-level options with no prefix which store their values directly into the
settings hash. Options with a prefix group their settings into a second level
...
still used to store the value in the settings hash).
...
action is called I<INSTEAD> of assigning the value into the settings hash! If
...
inside the action coderef.
    ...
    my $opt = App::Yath::Opt->new(..., action => sub {
        ...
        # Set the value in the settings hash
...
This callback is called AFTER all options have been processed. It is called for
all options in reverse order of their processing.

This is an options last chance to make syre everything is good. This is also a
...
An example callback:
    ...
    my $cb = sub {
        ...
        my $settings = $in{settings};    # The \%settings hash after processing
