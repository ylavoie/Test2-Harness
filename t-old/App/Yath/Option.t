...
subtest init => sub {
    like(dies { $CLASS->new() }, qr/You must specify 'title' or both 'field' and 'name'/, "Need title, or name and field");

    my $one = $CLASS->new(title => 'foo');
    ...
    for my $t (qw/b c m s d D default def multi-def multiple-default list-default array-default bool boolean count counter counting scalar string number multi multiple list array/) {
        ok(my $one = $CLASS->new(title => 'foo', type => $t), "type '$t' is valid");
        is($one->type, $t, "Did not modify type") if $t =~ m/^[bcsm]$/;
    }

    for my $t (qw/a e f h hash json/) {
        like(
            dies { $CLASS->new(title => 'foo', type => $t) },
            qr/Invalid type '\Q$t\E'/,
            "type '$t' is not valid"
        );
    }

    is($CLASS->new(title => 'foo', default => 1)->default,             1,     "Can use scalar as default (number)");
    is($CLASS->new(title => 'foo', default => 'foo')->default,         'foo', "Can use scalar as default (string)");
    is($CLASS->new(title => 'foo', default => sub { 1 })->default->(), 1,     "sub as default");

    like(
        dies { $CLASS->new(title => 'foo', default => {}) },
    ...
    $one = $CLASS->new(title => 'foo', category => 'foo', description => 'bar');
